...
public class ModdedJetpack : MonoBehaviour
{
    [Header("References")]
    ...
    ModdedPlayerCharacterController m_ModdedPlayerCharacterController;
    ...
    public bool isPlayergrounded() => m_ModdedPlayerCharacterController.isGrounded;
    ...
    void Start()
    {
        ...
        m_ModdedPlayerCharacterController = GetComponent<ModdedPlayerCharacterController>();
        DebugUtility.HandleErrorIfNullGetComponent<ModdedPlayerCharacterController, ModdedJetpack>(m_ModdedPlayerCharacterController, this, gameObject);

        m_InputHandler = GetComponent<PlayerInputHandler>();
        DebugUtility.HandleErrorIfNullGetComponent<PlayerInputHandler, ModdedJetpack>(m_InputHandler, this, gameObject);
    ...
    void Update()
    {
        ...
        else if (!m_ModdedPlayerCharacterController.hasJumpedThisFrame && m_InputHandler.GetJumpInputDown())
        {
        ...
        if(jetpackIsInUse)
        {
            ...
            // cancel out gravity
            totalAcceleration += m_ModdedPlayerCharacterController.gravityDownForce;

            if (m_ModdedPlayerCharacterController.characterVelocity.y < 0f)
            {
                // handle making the jetpack compensate for character's downward velocity with bonus acceleration
                totalAcceleration += jetpackDownwardVelocityCancelingFactor;
            }

            // apply the acceleration to character's velocity
            m_ModdedPlayerCharacterController.characterVelocity += transform.up * totalAcceleration * Time.deltaTime;
        ...
        else
        {
            ...
            {
                float refillRate = 1 / (m_ModdedPlayerCharacterController.isGrounded ? refillDurationGrounded : refillDurationInTheAir);
